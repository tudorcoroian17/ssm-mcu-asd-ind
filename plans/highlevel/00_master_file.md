...
- **Parameter count — not reported by the paper**. An earlier summary listed ~2.5M–6.5M as "confirmed"; that figure was this project's own estimate, not something the paper states. The paper reports no parameter count at all. The estimate is still a reasonable order-of-magnitude anchor for the red flag below, but it must not be cited as a published number. **Red flag worth sitting with:** at float32 that's 10–26MB; even aggressively int8-quantized, 2.5–6.5MB. The committed RP2040 target (Section 4) has 2MB total flash — **this architecture, as published, almost certainly does not fit at all on the leanest target, and may be tight even on the H7S3L8** depending on its exact flash size. "Shrink this specific architecture enough to fit" likely needs to be an explicit, first-class methodology step, not an assumed given (see Section 17).
- **Anomaly head — corrected after reading the paper directly**. Not a fusion of a reconstruction score and a latent-distance score. It fuses the **final-layer embedding with a learned, softmax-weighted combination of pooled intermediate-layer embeddings** — fusion across encoder depth, not across score type. L2-normalized, then a small MLP. There is no reconstruction pathway, no decoder, and no S_recon anywhere in the paper. Test-time scoring is cosine distance to a learned SVDD-style center — structurally closer to this project's Option 3 than earlier summaries assumed.
- **Training regime — corrected**. Two-stage and supervised, not self-supervised. Stage 1 is supervised classification pretraining across seven machine types. Stage 2 freezes most of the backbone and fine-tunes the last K blocks plus a new anomaly head with a pseudo-outlier-exposure loss (SVDD-style: pull normal embeddings toward a learned center via cosine similarity, push synthetic pseudo-outliers away), plus a domain-adversarial loss via a gradient-reversal layer.
- **Implication for this project's chosen Option 3 — corrected**. The fusion-gap concern does not hold as originally framed. Since the published head fuses across depth rather than reconstruction-vs-distance, there is no S_recon component this project is missing relative to theirs. What actually differs is (a) this project scores from a single final embedding rather than a depth-fused one, and (b) the training signal — self-supervised next-frame prediction on one machine type versus supervised classification pretraining plus pOE plus domain-adversarial fine-tuning on seven. Any accuracy gap against the published number is better explained by that difference, which is a more honest and more interesting trade-off to report than the original "missing reconstruction pathway" framing.
- **Reported numbers (anchor for Phase 1 §1.8)**. Machine-specific model, harmonic-mean TOTAL 0.722. ToyCar AUC: source 0.894 / target 0.848. ToyTrain AUC: source 0.736 / target 0.620. DCASE 2025 baseline autoencoder — ToyCar 0.790/0.725, ToyTrain 0.673/0.598. Caveat: their preprocessing (128-mel, 10s clips fixed to 1024 frames, seven-machine pretraining) does not match this project's pipeline.
- **No MCU or embedded deployment attempted**. Trained and evaluated on a single RTX 3090; reports 22.7 ms/clip GPU inference latency. No parameter count, no quantization, no hardware target. This gap remains genuinely open.
...
#### 10.1 Threshold selection for Option 3 — COMMITTED: use all three approaches together
...
**Note on the published paper's own head (corrects the earlier Section 2 summary)**. The DCASE SSM paper's anomaly head is not a reconstruction-vs-distance fusion — it is closer to a depth-fused variant of this option space: pool embeddings from several encoder layers, combine with learned weights, score by distance to a center. Structurally it sits nearer Option 3 (distance-based, no decoder) than Option 1. The gap between this project's head and theirs is single-layer-embedding-distance versus multi-layer-fused-embedding-distance, plus a very different training regime (Section 2) — not "no reconstruction versus reconstruction."
...
12. **~~Fusion-gap framing for the writeup~~** — RESOLVED AS MISDESCRIBED. The premise was wrong: the published head fuses across encoder depth, not reconstruction-versus-distance (Section 2, corrected). There is no reconstruction pathway in this project's head that needs apologising for, because there is none in theirs either. What remains reportable is a different comparison — single-embedding distance versus depth-fused embedding distance, and self-supervised prediction versus two-stage supervised pretraining. The `S_recon` ablation proposed in `01_design_decisions.md` §4 is still worth running on its own merits; it just does not close the gap this item originally described.
...
1. **GPU prototype phase - COMPLETE (August 2026)** — SSM backbone + Option 3 head, trained and evaluated on GPU, unquantized, ToyCar IND-only, four LOSO folds at three seeds. Ceiling number recorded: mean AUC 0.9770 (`knn_full`, mean pooling, averaged across folds and seeds); deployment-realistic recommendation 0.9599 (`knn_clustered_16`, 4 KB head). See `findings/130_evaluation_of_default_model_across_folds.md`.
