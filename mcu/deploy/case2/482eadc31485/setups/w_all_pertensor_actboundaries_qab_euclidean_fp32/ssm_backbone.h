...
/* CLASSIC (selective=False) backbone, fake-quant schemes. Identical in
 * shape to the selective ssm_backbone.h -- the recurrence differs, the
 * state does not. SSM_D_INNER is expand*SSM_D_MODEL as always, so a config
 * with expand=2 doubles conv_hist relative to an expand=1 selective model;
 * check the .ssm_state_dtcm budget in the linker script before flashing. */
 ...
 * raw frame for the existing debug transmission. */
