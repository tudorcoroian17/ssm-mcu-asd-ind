...
static inline int8_t ssm_requantize_r(float real, float inv_scale) {
...
static void ssm_block_step_true_int8(
	...
	for (int c = 0; c < SSM_D_INNER; c++) {
		u_raw_q[c] = ssm_requantize_r(u_raw_real[c], inv_s_conv_out);
		z_q[c] = ssm_requantize_r(z_real[c], inv_s_z_gate);
	}
	...
	for (int c = 0; c < SSM_D_INNER; c++) {
		...
		int8_t conv_out_q = ssm_requantize_r(conv_real, inv_s_conv_out);
	...
	for (int c = 0; c < SSM_D_INNER; c++) {
		int8_t delta_raw_q = ssm_requantize_r(delta_raw_real[c], inv_s_dt_proj_out);
	...
	for (int c = 0; c < SSM_D_INNER; c++) {
		...
		for (int n = 0; n < SSM_D_STATE; n++) {
			...
			int8_t A_bar_q = ssm_requantize_r(A_bar_real, inv_s_A_bar);

			float B_bar_real = (float) ((int32_t) delta_q[c] * (int32_t) B_q[n])
					* w->s_delta * w->s_B;
			int8_t B_bar_q = ssm_requantize_r(B_bar_real, inv_s_B_bar);
			...
			float new_h_q_f = roundf(new_h_real * inv_s_h);
		...
		int8_t y_scan_q = ssm_requantize_r(y_scan_real, inv_s_y_scan);
		...
		y_gated_q[c] = ssm_requantize_r(y_gated_real, inv_s_y_gated);
	}
