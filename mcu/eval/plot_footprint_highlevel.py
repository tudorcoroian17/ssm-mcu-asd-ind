"""Plots MCU footprint (flash and RAM) by high-level component, split into
subplots by selectivity role (selective vs. classic).

Reads mcu/eval/online/<board>/footprint_highlevel_<board>.csv, which is long
format: each (case, model, setup) is spread across one row per high-level
group (backbone, head, frontend, ...), with flash_bytes and ram_bytes columns.
This script produces two images -- one for flash, one for RAM -- each with
bars stacked by group and values shown in KB.

Bars are horizontal: setup on the y-axis, footprint (KB) on the x-axis. Both
subplots share the full union of setups present in the board's footprint
file, so a setup that exists for only one role still gets a row in both
subplots -- empty where no data exists for that model. Models within a role
occupy a fixed vertical slot regardless of whether they have data for a given
setup, so a missing model-setup pair shows up as a blank slot rather than
being hidden by re-centering the remaining bars. Hatch patterns -- and the
vertical stacking order that goes with them -- are assigned globally by
config signature (the fields that vary across models), so the same
underlying configuration always gets the same hatch and the same
above/below position in every subplot.

Note: this CSV has an "architecture" column, but it is a hardcoded
placeholder in footprint_breakdown.py (always "selective") rather than a
real per-model flag -- see the ARCHITECTURE constant there. Selectivity role
is determined the same way as in plot_timing.py: from each model's
ckpt_descriptor.json, not from this column.
"""
...
# High-level footprint groups to stack along the horizontal axis, in the same
# order footprint_breakdown.py defines as GROUP_ORDER: (group, legend label, color).
GROUP_ORDER = [
    ("backbone", "Backbone", "#3274a1"),
    ("head", "Head", "#e1812c"),
    ("frontend", "Frontend", "#c63c32"),
    ("dsp_library", "DSP Library", "#3a923a"),
    ("platform", "Platform", "#9467bd"),
    ("instrumentation", "Instrumentation", "#8c564b"),
    ("reserved", "Reserved", "#7f7f7f"),
    ("unclassified", "Unclassified", "#bcbd22"),
]

BYTES_PER_KB = 1024.0

# (CSV column, axis/title label, output filename)
METRICS = [
    ("flash_bytes", "Flash (KB)", "flash_breakdown.png"),
    ("ram_bytes", "RAM (KB)", "ram_breakdown.png"),
]
...
def load_model_config(case, model_hash, project_root):
    ...
    Args:
        case: Case identifier as it appears in the footprint CSV, e.g. "case1".
        model_hash: Model hash as it appears in the footprint CSV's "model" column.
...
def build_selectivity_groups(df, project_root):
    """Groups (case, model) combinations by selectivity role.
    ...
    even when they don't share identical setups. Role comes from each
    model's ckpt_descriptor.json, not from this CSV's "architecture"
    column -- that column is a hardcoded placeholder, not a real flag.

    Args:
        df: Footprint results; must contain "case" and "model" columns.
        project_root: Root of the ssm-mcu-asd-ind repository.
...
def plot_group(ax, df, case, role, members, varying_keys, setup_positions,
               hatch_by_hash, rank_by_hash, metric_col):
    ...
    always drawn above the hatched one, in every subplot. Each stacked
    segment's value comes from a separate row of this long-format CSV
    (one row per high-level group), looked up by group name rather than by
    column, since the stages aren't already separate columns here.

    Args:
        ax: Matplotlib axes to draw into.
        df: Footprint results for one metric's worth of plotting.
        ...
        setup_positions: Dict mapping every setup (across all subplots) to
            its shared y-axis position.
        ...
        metric_col: Either "flash_bytes" or "ram_bytes".

    Returns:
        Dict mapping group label to bar artist, for the shared figure legend.
    """
    ...
    for model_hash in model_hashes:
        ...
        for setup, setup_rows in model_rows.groupby(SETUP_COL):
            y = setup_positions[setup] + offset_by_hash[model_hash]
            value_by_group = setup_rows.set_index(GROUP_COL)[metric_col]
            left = 0.0
            for group_key, group_label, color in GROUP_ORDER:
                val_bytes = value_by_group.get(group_key, 0.0)
                val_kb = float(val_bytes) / BYTES_PER_KB
                bar = ax.barh(
                    y,
                    val_kb,
                    left=left,
                ...
                left += val_kb
                if group_label not in group_legend_handles:
                    group_legend_handles[group_label] = bar
    ...
    return group_legend_handles


def generate_footprint_chart(df, metric_col, metric_label, output_path, project_root):
    """Renders one footprint metric as subplots side by side, one per
    (case, selectivity role) group, with a shared setup axis and a shared,
    locked value axis.

    Args:
        df: Footprint results loaded from a board's footprint_highlevel CSV.
        metric_col: Either "flash_bytes" or "ram_bytes".
        metric_label: Axis/title label, e.g. "Flash (KB)".
    ...
    if not groups:
        raise ValueError("No models found in the provided footprint data.")
    ...
    # Every setup in the (filtered) file, shared across every subplot.
    ...
    fig_height = max(8, len(all_setups) * 0.40)
    ...
    for ax, (case, role) in zip(axes_flat, ordered_keys):
        handles = plot_group(
            ...
            setup_positions, hatch_by_hash, rank_by_hash, metric_col,
        )
        all_group_handles.update(handles)
        ax.set_xlabel(metric_label, fontsize=10, fontweight="bold")

    axes_flat[0].set_ylabel("Setup ID", fontsize=11, fontweight="bold")

    fig.suptitle(
        f"{metric_label} by Setup, Grouped by Selectivity",
        fontsize=14,
        fontweight="bold",
    )
    fig.legend(
        all_group_handles.values(),
        all_group_handles.keys(),
        title="High-Level Group",
        loc="upper center",
        ncol=min(len(all_group_handles), 4),
...
if __name__ == "__main__":
    ...
    footprint_file = (
        PROJECT_ROOT / "mcu" / "eval" / "online" / args.board
        / f"footprint_highlevel_{args.board}.csv"
    )
    footprint_df = pd.read_csv(footprint_file)

    out_dir = PROJECT_ROOT / "mcu" / "eval" / "online" / args.board / "footprint"
    for metric_col, metric_label, filename in METRICS:
        generate_footprint_chart(footprint_df, metric_col, metric_label, out_dir / filename, PROJECT_ROOT)