...
def run_case_eval(held_out_case, checkpoint, cfg, device):
    out_dir = PROJECT_ROOT / 'runs' / f'case{held_out_case}' / 'eval'
    ...
    for pooling_mode in POOLING_MODES:
        ...
        plot_embedding_pca(
            ...
            out_path=out_dir / f'embedding_pca_{pooling_mode}.png',
        )
        ...
        for head_name, head_fn in DISTANCE_HEADS.items():
            ...
            plot_score_distribution(
                ...
                out_path=out_dir / f'score_distribution_{suffix}.png',
            )
            plot_roc(
                scores, test_labels, auc,
                title=f'case{held_out_case} [{pooling_mode}/{head_name}]: ROC',
                out_path=out_dir / f'roc_curve_{suffix}.png',
            )
...
if __name__ == "__main__":
    ...
    for case in cases:
        print(f'\n=== held_out_case {case} ===')
        results = run_case_eval(case, checkpoints[case - 1], cfg, device)
